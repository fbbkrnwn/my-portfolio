...
function bgAnimasi(){
    ...
    for(let i = 0; i < rows; i++){
        for(let j = 0; j < cols; j++){
            ...
            document.querySelector(".background-animasi").appendChild(div);
        }
    }
}
bgAnimasi();
...
filterBtnsContainer.addEventListener("click", (e) =>{
    if(e.target.classList.contains("btn-portfolio-filter") && !e.target.classList.contains("active")){
...
filterItems(document.querySelector(".btn-portfolio-filter.active"));
...
document.querySelector(".pp-btn-close").addEventListener("click", togglePopup);
function portfolioItemDetails(){
    document.querySelector(".pp-thumbnail img").src = 
    portfolioItems[portfolioItemIndex].querySelector("img").src;

    document.querySelector(".pp-header h3").innerHTML = 
    portfolioItems[portfolioItemIndex].querySelector(".title-portfolio-item").innerHTML;
    
    document.querySelector(".pp-body").innerHTML = 
    portfolioItems[portfolioItemIndex].querySelector(".details-portfolio-item").innerHTML;
    
    document.querySelector(".pp-counter").innerHTML = 
    `${portfolioItemIndex+1} of ${portfolioItems.length} ( <span title="category">${document.querySelector(".btn-portfolio-filter.active").innerHTML}</span> )`;
}
